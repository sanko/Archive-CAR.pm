...
=head2 C<write($filename, $roots, $blocks, [ $version ])>

Writes a new CAR file. C<$roots> is an arrayref of CIDs, and C<$blocks> is an arrayref of hashes containing C<cid> and
C<data>. C<$version> defaults to 1.
...
Returns an array reference of L<Archive::CAR::CID> objects that are the designated entry points for this archive.

=head2 C<blocks()>

Returns an array reference of hashes, where each hash contains C<cid> (an L<Archive::CAR::CID> object) and C<data> (the
raw binary block).

=head1 SEE ALSO

L<https://ipld.io/specs/transport/car/>, L<Archive::CAR::CID>, L<Archive::CAR::v1>, L<Archive::CAR::v2>
